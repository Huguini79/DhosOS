...
DIRS = ./build ./bin ./build/io ./build/idt ./build/memory ./build/memory/heap ./build/disk ./build/string ./build/memory/paging ./build/gdt ./build/keyboard ./build/pci

$(shell mkdir -p $(DIRS))


all: ./bin/boot.bin ./bin/kernel.bin
	rm -rf ./bin/DhosOS.img
	rm -rf ./bin/DhosOS.iso
	rm -rf ./bin/os.bin
	dd if=/dev/zero of=./bin/DhosOS.img bs=512 count=400
	dd if=./bin/boot.bin of=./bin/DhosOS.img bs=512 count=1 conv=notrunc
	dd if=./bin/boot.bin >> ./bin/os.bin
	dd if=./bin/boot.bin >> ./bin/DhosOS.iso
	dd if=./bin/kernel.bin of=./bin/DhosOS.img bs=512 seek=1 conv=notrunc
	dd if=./bin/kernel.bin >> ./bin/os.bin
	dd if=./bin/kernel.bin >> ./bin/DhosOS.iso
	dd if=hola.txt of=./bin/DhosOS.img bs=512 seek=50 conv=notrunc
	dd if=/dev/zero bs=512 count=100 >> ./bin/os.bin
	dd if=/dev/zero bs=512 count=100 >> ./bin/DhosOS.iso
